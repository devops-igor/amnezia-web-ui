...
class TelegramAPI:
    ...
    async def edit_message(
        self, chat_id, message_id, text: str, reply_markup=None, parse_mode="HTML"
    ):
        import json
...
async def _handle_get_config(
    ...
    loading_result = await api.send_message(
        chat_id, f"⏳ Fetching config for <b>{conn_name}</b>..."
    )
    ...
    try:
        ...
        if not config:
            if loading_msg_id:
                await api.edit_message(
                    chat_id, loading_msg_id, "❌ Failed to retrieve configuration."
                )
            return
        ...
        if len(config) <= MAX_LEN:
            ...
            for i, chunk in enumerate(chunks, 1):
                await api.send_message(
                    chat_id,
                    f"<b>📄 Configuration (part {i}/{len(chunks)}):</b>\n<pre>{chunk}</pre>",
                )
...
async def _dispatch(
    api: TelegramAPI, update: dict, load_data_fn: Callable, generate_vpn_link_fn: Callable
):
    # --- Text messages ---
    if "message" in update:
        msg = update["message"]
