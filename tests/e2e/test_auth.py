...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_login_page_loads(page: Page, base_url: str) -> None:
    """GET /login returns 200 and shows the login form."""
    await page.goto(f"{base_url}/login")
    await page.wait_for_load_state("networkidle")

    # Page should have the login form
    assert await page.locator("input#username").is_visible()
    assert await page.locator("input#password").is_visible()
    assert await page.locator("form#loginForm").is_visible()


@pytest.mark.e2e
@pytest.mark.asyncio
async def test_login_success(page: Page, base_url: str, admin_user: str, admin_pass: str) -> None:
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_login_failure(page: Page, base_url: str) -> None:
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_login_rate_limiting(page: Page, base_url: str) -> None:
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_csrf_protection(page: Page, base_url: str) -> None:
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_logout(authenticated_page: Page, base_url: str, csrf_token: str) -> None:
